...
%% 运行 PCA
%     trainset_o = trainset - repmat(mean(trainset,2), [1,Ntrain]);
%     testset = testset - repmat(mean(testset,2), [1,Ntest]);
%     XXt = trainset_o*trainset_o'; % D*D
%     [E, Eval] = eig(XXt);
%     Eval = diag(Eval); % D*1
%     [~,idx_e] = sort(-Eval); % 降序排序
%     E = E(:, idx_e);
    [E,~,~] = svd(trainset - repmat(mean(trainset,2), [1,Ntrain]));
    for d=lb_d:ub_d
